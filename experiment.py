...
def part_1():
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        marker_positions = ps3.find_markers(scene, template, False)

        for marker in marker_positions:
            mark_location(scene, marker)

        save_image(img_out, scene) # save_image(img_out, scene) not sure if this should be commented out
...
def part_2():
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        markers = ps3.find_markers(scene, template, False)
        image_with_box = ps3.draw_box(scene, markers, 3)

        save_image(img_out, image_with_box)  # NOT SURE if this should be cleared
...
def part_3():
    ...
    for img_in, img_out in zip(input_images, output_images):
        ...
        save_image(img_out, projected_img) # save_image(img_out, projected_img) not sure if this should be commented out
...
def part_6():
    ...
    video_file = "ps3-4-a.mp4"
    their_video = os.path.join(VID_DIR,video_file)
    my_video_file = "my-ad.mp4"  # Place your video in the input_video directory
    my_video = os.path.join(VID_DIR,my_video_file)
    frame_ids = [355, 555, 725]
    fps = 40
    output_prefix = "ps3-6-a"
    # referencing previously provided funciton helper_for_part_4_and_5()
    # launch video
    their_image_gen = ps3.video_frame_generator(their_video)
    my_image_gen = ps3.video_frame_generator(my_video)
    # get images from video
    their_image = their_image_gen.__next__()
    their_h, their_w, their_d = their_image.shape
    ...
    while their_image is not None:
        ...
        their_image = ps3.project_imageA_onto_imageB(my_advert_image, their_image, homography)
        # print("the image after the merge\n",their_image)
        #
        ...
        my_advert_image = my_image_gen.__next__()
        their_image = their_image_gen.__next__()
        if my_advert_image is None:
            print("the advert ran out, trying to replay it")
            my_video = os.path.join(VID_DIR, my_video_file)
            my_image_gen = ps3.video_frame_generator(my_video)
            my_advert_image = my_image_gen.__next__()
            # my image as advert
